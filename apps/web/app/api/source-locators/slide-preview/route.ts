...
export async function GET(request: Request) {
  ...
  if (locator.type !== "slide")
    return apiError(400, "SLIDE_LOCATOR_REQUIRED", "该来源不是幻灯片定位");
  try {
    ...
    try {
      ...
      return apiError(404, "SLIDE_PREVIEW_UNAVAILABLE", "幻灯片派生内容暂时不可读取");
    }
    const preview = selectSlidePreview(document, locator);
    if (!preview) return apiError(404, "SLIDE_NOT_FOUND", "该幻灯片定位暂时不可读取");
    return Response.json({ preview });
  } catch {
    return apiError(503, "SLIDE_PREVIEW_UNAVAILABLE", "幻灯片预览服务暂时不可用，请稍后重试");
  }
}
