...
export async function GET(request: Request) {
  ...
  if (locator.type !== "sheet")
    return apiError(400, "SHEET_LOCATOR_REQUIRED", "该来源不是表格范围定位");
  try {
    ...
    try {
      ...
      return apiError(404, "SHEET_PREVIEW_UNAVAILABLE", "表格派生内容暂时不可读取");
    }
    const preview = selectSheetPreview(document, locator);
    if (!preview) return apiError(404, "SHEET_RANGE_NOT_FOUND", "该表格范围暂时不可读取");
    return Response.json({ preview });
  } catch {
    return apiError(503, "SHEET_PREVIEW_UNAVAILABLE", "表格预览服务暂时不可用，请稍后重试");
  }
}
