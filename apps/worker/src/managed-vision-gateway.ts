...
export async function createManagedVisionGateway(
  ...
  for (const row of rows) {
    if (!row.capabilities.includes("vision")) continue;
    if (!isVisionProviderLocationCompatible(row.location, dataPolicy)) continue;
