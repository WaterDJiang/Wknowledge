...
> 2026-08-17 决策更新：本文的“Pi 候选/内部 Loop 默认实现”已由 [Pi 核心组件升级计划](pi-core-component-platform-upgrade-v1.md) 与 [ADR 0004](../design/0004-pi-core-component-runtime.md) 取代。领域安全、来源、审批和学习确认约束继续有效；后续实施顺序以新计划为准。
...
| ------------- | ------------------------------------------------ | ----------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Pi agent core | `AgentCoreAdapter` 后的目标默认核心              | 工具循环、事件、停止、消息转换、上下文裁剪      | 将 Pi 的会话、默认权限、文件系统或模型配置作为业务真相源                    | 锁定版本/commit、许可证、依赖树、生命周期脚本、摘要、20 条合成轨迹与替换测试通过后切换；否则继续保留内部 Loop |
...
| P1     | M5-13 至 M5-16                      | Pi Adapter、Agent Skills 资源层、Tool Registry 与会话事件桥               | 不使用 Pi 默认文件/进程/网络权限，不绕过模型网关与领域端口   | 供应链归档、20 条轨迹、停止恢复、来源/权限/审计与内部回退门禁     |
...
### A. Pi 核心迁移（M5-13 至 M5-16；历史基线 M5-00）

交付：

- 同一脚本化 Provider 下的 Pi Adapter 与最小内部 Adapter；替换门禁通过前保持双路径。
- 流式文本、串行/并行工具、停止、失败、上下文裁剪和事件顺序的 20 条确定性轨迹。
- Pi/OpenCode 的许可证、版本、依赖树、生命周期脚本、更新频率与安全边界审查。
- Claude Code 的公开交互、审批、Hook 和工具状态对照记录。

退出条件：Adapter 替换不修改 `AgentSession`、`ToolCall`、`SkillRun`、`ModelCall` 或学习数据 Schema；Pi 路径通过后进入默认配置，内部 Loop 随 M7-13 删除生产导出并保留短期测试夹具。
...
M3 可浏览/可引用 Wiki
→ M4 来源预览与相应媒体定位
→ M5-13 至 M5-16 Pi 核心与 Agent Skills 迁移（可与 M3 黄金集准备并行）
...
4. 按升级计划完成 `M5-13` 至 `M5-16`；Pi 未经供应链与替换门禁不得进入默认生产路径，OpenCode/Claude Code 只保持上述边界。
