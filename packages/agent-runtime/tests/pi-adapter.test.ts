...
const internal = new InternalAgentCoreAdapter();
const pi = new PiAgentCoreAdapter();

async function piEvents(script: readonly AgentCoreScriptEvent[], signal?: AbortSignal) {
  return collectAgentCoreEvents(pi, { runId: "run-fixture", script, signal });
}

const validTrajectories: Array<{ name: string; script: AgentCoreScriptEvent[] }> = [
  {
  ...
  {
    name: "emits text before tools",
    script: [
      { type: "assistant.delta", text: "checking" },
      {
        type: "tool.requested",
        toolCallId: "search-1",
        tool: "knowledge.search",
  ...
  {
    name: "emits text before a failure",
    script: [
  ...
  {
    name: "emits multiple deltas in one answer",
    script: [
  ...
  {
    name: "runs a second tool round before answering",
    script: [
      {
        type: "tool.requested",
        toolCallId: "search-1",
...
const invalidTrajectories: Array<{ name: string; script: AgentCoreScriptEvent[] }> = [
  {
...
describe("pi agent core adapter trajectory equivalence", () => {
  it.each(validTrajectories)("$name", async ({ script }) => {
    const [internalResult, piResult] = await Promise.all([
      collectAgentCoreEvents(internal, { runId: "run-fixture", script }),
      piEvents(script)
    ]);
    expect(piResult).toEqual(internalResult);
  });

  it.each(validTrajectories)("keeps a unique terminal on $name", async ({ script }) => {
    const result = await piEvents(script);
    expect(result[0]).toEqual({ type: "run.started", runId: "run-fixture" });
    const terminals = result.filter((event) =>
      /^run\.(completed|stopped|failed)$/.test(event.type)
    );
    expect(terminals).toHaveLength(1);
    expect(result.at(-1)).toEqual(terminals[0]);
    expect(result.every((event) => event.runId === "run-fixture")).toBe(true);
  });

  it.each(invalidTrajectories)("$name", async ({ script }) => {
    await expect(piEvents(script)).rejects.toThrow("AGENT_CORE_TRACE_INVALID");
  });
});

describe("pi agent core adapter stop semantics", () => {
  it("stops immediately when the signal was already aborted", async () => {
    const controller = new AbortController();
    controller.abort();
    await expect(piEvents([{ type: "run.completed" }], controller.signal)).resolves.toEqual([
  ...
  it.each(["", " "])("rejects an invalid run id %j", async (runId) => {
    await expect(
      collectAgentCoreEvents(pi, { runId, script: [{ type: "run.completed" }] })
    ).rejects.toThrow("AGENT_CORE_TRACE_INVALID");
  });

  it("registers no tools and no gateways beyond the validated script", () => {
    expect(pi.id).toBe("pi-agent-core-0.84.2");
    const ownKeys = Object.getOwnPropertyNames(pi);
    expect(ownKeys).toEqual(["id"]);
  });
});
